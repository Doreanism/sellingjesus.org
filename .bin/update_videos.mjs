...
// Fetch every page of a playlist, merging in any existing per-video data
const fetch_all_videos = async (playlist_id, api_key) => {
    const videos = []
    let next_token = null
    while (true){
...
// Fetch fresh data, but fall back to the committed JSON if the API is unavailable
const api_key = process.env.YOUTUBE_API_KEY
const playlist_id = settings[`youtube_playlist_${playlist_type}`]
let videos
try {
    videos = await fetch_all_videos(playlist_id, api_key)
} catch (error) {
    // Quota/network errors shouldn't break the build; keep the existing videos
    console.warn(`WARNING: Skipping ${playlist_type} video update: ${error.message}`)
    process.exit(0)
}
